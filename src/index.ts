...
import { determineGenre, toIpa } from "./wqle";
...
client.on("messageCreate", async (message) => {
  ...
  const dictChRef: ChannelRef = [dictChSf, "channel"];
  ...
  if (channelId === classroomChSf) {
    await channel.send(
      await HandleFlashcardsMessage([author.id, "user"], content)
    );
    return;
  }
